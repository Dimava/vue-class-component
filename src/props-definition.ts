...
type ExtractPropType<T>
	= T extends Array<infer V> ? ExtractPropTypeSingle<V>
	// : T extends (Constructor & ((...args: any[]) => infer V)) ? V
	: T extends NumberConstructor ? number
	: T extends StringConstructor ? string
	: T extends BooleanConstructor ? boolean
...
export type ExtractPropTypesIn<T>
	= { [K in keyof T]: ExtractPropType<T[K]> }



type ExtractPropDefault<T>
...
export interface PropOptionsWith<T, D, R extends boolean> extends PropOptions<T, D> {
	required: R,
...
function makeValidator<T extends ValidPropDefinition>(meta: T, key: string): (value: unknown) => value is ExtractPropType<T> {
...
export interface PropsDefinition {
	[propName: string]: ValidPropDefinition
}

export function makePropClass<T extends PropsDefinition>(propsMeta: T): { new(): ConstructPropClass<T> } {
