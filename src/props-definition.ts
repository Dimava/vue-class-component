...
import { PropOptions, WithDefault } from './props'
...
export type ExtractPropTypeSingle<T> =
  ...
  T extends NumberConstructor
    ...
    ? boolean
    : T extends abstract new (...args: any) => infer V
    ? V
    : T extends undefined
    ? never
    : T // null, 123, 'wasd', {}, ()=>{}

export type ExtractPropType<T> = T extends readonly (infer V)[]
  ? ExtractPropTypeSingle<V>
  : // : // : T extends (Constructor & ((...args: any[]) => infer V)) ? V
  // T extends NumberConstructor
  // ? number
  // : T extends StringConstructor
...
export type ExtractPropDefault<T> = T extends readonly [
  ...any[],
  infer P,
  infer D
]
  ? D extends null
    ? D // last null
    : P extends Exclude<SimpleType, undefined>
    ? never // enums
    : D extends SimpleType
    ? D
  ...
  ? never // FIXME: single constructor - what should it do?
  : unknown // FIXME: function - what should it do?
...
export type ConstructSingleOption<T> =
  // [String]
  never extends ExtractPropDefault<T>
    ? PropOptionsWith<ExtractPropType<T>, never, false>
    : // [String, null]
    null extends ExtractPropDefault<T>
    ? PropOptionsWith<ExtractPropType<T>, ExtractPropDefault<T>, true>
    : undefined extends ExtractPropDefault<T>
    ? PropOptionsWith<ExtractPropType<T>, ExtractPropDefault<T>, true>
    : // [String, "default-value"]
      PropOptionsWith<ExtractPropType<T>, ExtractPropDefault<T>, false>

export type ConstructPropClass<T> = {
  [K in keyof T as ExtractPropDefault<T[K]> extends never
    ? never
    : K]?: WithDefault<ExtractPropType<T[K]>>
} & {
  [K in keyof T as ExtractPropDefault<T[K]> extends never
    ? K
    : never]: ExtractPropType<T[K]>
}
